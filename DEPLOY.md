# FinFlow 分散式邊緣運算系統部署指南（v24，修正 /start-node 組網址時漏讀 DOMAIN_NAME 的問題）

> **v24** 內容變更摘要見文末「變更紀錄摘要」的 v24 小節；編號規則沿用 v22（見下方
> 說明未變動）。
...
2. 部署這次修正、重新 `/start-node` 之後，直接去 Kaggle 網站看該次 kernel 的
   Logs/Output，確認有沒有印出「[階段一] 偵測到 GPU」「[階段一] 已回報開機
   狀態」這兩行——如果連這兩行都沒出現，代表卡在 GPU 偵測本身（`nvidia-smi`
   抓不到），跟連線／防火牆無關，是另一個問題；如果有印出來但後面接著看到
   連線失敗的訊息（`ConnectionError`／`SSLError`／`timeout`），才是這次修正
   真正對症的情境。
