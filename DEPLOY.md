# FinFlow 分散式邊緣運算系統部署指南（v21，Kaggle 節點改為兩階段啟動＋GPU 資訊回報）
...
### 3. 註冊管理指令（`/start-node`、`/stop-node`、`/load-node`、`/list-nodes`）
...
**GPU 型號可以指定「種類」**：`kaggle kernels push` 有一個 `--accelerator` 參數
（例如 `NvidiaTeslaT4`），可以在 `NODE_PLATFORM_MAP` 這個節點的設定裡加
`"accelerator": "NvidiaTeslaT4"` 來指定，`node_controllers/kaggle.py` 會自動帶進
push 指令；不填就交給 Kaggle 依當下配額自動分配（張數的部分見上面「為什麼要拆
兩階段」的說明）。`enable_gpu` 跟 `enable_tpu` 是完全獨立的兩個開關（見
