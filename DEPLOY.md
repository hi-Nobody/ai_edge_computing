...
修正第三個、也是最隱蔽的心跳空窗：`worker_loop()` 領到任務後呼叫
`run_inference()` 是同步阻塞呼叫，推論花多久主執行緒就卡多久，原本
`if loop_count % 15 == 0: register_node(...)` 這個心跳檢查完全不會在推論
期間被執行到。實測用完整的 Kaggle log 比對過真實時間軸：一次 38 秒的推論，
加上它剛好卡在心跳週期中段，兩者疊加湊出將近 70 秒的空窗，超過 Oracle 端
`NODE_DEAD_AFTER_SEC`（60 秒），導致節點明明在正常工作、`/list-nodes` 卻
短暫顯示離線。這是繼 v25（階段一等待迴圈）、v28（下載/暖機階段）之後第三個
獨立的心跳空窗，但成因是同一個模式：主執行緒被同步阻塞呼叫卡住時，任何
「只在迴圈跑到下一輪才觸發」的心跳邏輯都會失效。這次不再逐一補洞，改成
背景心跳執行緒貫穿整個節點生命週期（開機到關閉都是同一條執行緒），不管
主執行緒卡在下載模型、暖機、還是處理任務，心跳都不受影響；`register_node()`
新增的 `status` 參數機制沿用 v28 的設計不變。

| 檔案 | 變更 |
|------|------|
| `edge-worker/bootstrap.py` | `_loading_heartbeat_loop()` 改名並擴大為 `_heartbeat_daemon()`，從 `main()` 一開始（GPU 偵測完）啟動、貫穿到 `worker_loop()` 整個生命週期，不再是「只涵蓋載入階段、載入完就停掉」；新增模組層級的 `_node_state = {"status": "loading"}` 讓背景執行緒能讀到目前該回報什麼狀態，`main()` 在暖機完成後改成更新 `_node_state["status"] = "running"`；`interval_sec` 從 25 秒收緊到 20 秒，多留一點緩衝；`worker_loop()` 移除原本 `if loop_count % 15 == 0: register_node(...)` 這段（在推論阻塞期間完全不會被執行到，是這次空窗的直接成因），連帶移除已經沒有用途的 `loop_count` 變數 |
