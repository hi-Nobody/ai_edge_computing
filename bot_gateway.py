...
DISCORD_PUBLIC_KEY = os.environ.get("DISCORD_PUBLIC_KEY", "")  # Developer Portal -> General Information
# 註：DISCORD_BOT_TOKEN 只有註冊 Slash Command 的一次性腳本
# （register_discord_commands.py）需要，本服務執行期間不需要 bot token，
# 因為 followup 訊息編輯用的是 interaction token，不是 bot token。
...
def verify_discord_signature(body: bytes, signature: Optional[str], timestamp: Optional[str]) -> bool:
    """Discord 用 Ed25519 簽章驗證（不是 HMAC），驗證 timestamp+body 是否經過
    DISCORD_PUBLIC_KEY 對應的私鑰簽署。"""
    # ↓↓↓ 暫時性除錯 log，排查完 401 問題後記得移除或註解掉 ↓↓↓
    log.warning(
        "DEBUG discord verify: has_sig=%s has_ts=%s key_len=%d body_len=%d",
        bool(signature), bool(timestamp), len(DISCORD_PUBLIC_KEY), len(body),
    )
    if not signature or not timestamp or not DISCORD_PUBLIC_KEY:
        log.warning("DEBUG discord verify: 缺少 signature/timestamp/PUBLIC_KEY，直接判定失敗")
        return False
    try:
        verify_key = VerifyKey(bytes.fromhex(DISCORD_PUBLIC_KEY))
        verify_key.verify(timestamp.encode("utf-8") + body, bytes.fromhex(signature))
        log.warning("DEBUG discord verify: 簽章驗證成功")
        return True
    except BadSignatureError:
        log.warning("DEBUG discord verify: BadSignatureError —— 金鑰格式沒問題，"
                    "但簽章跟 body/timestamp 對不起來（很可能是 body 在中間被改過，"
                    "或 DISCORD_PUBLIC_KEY 不是這個 Application 真正的 Public Key）")
        return False
    except ValueError as e:
        log.warning("DEBUG discord verify: ValueError（hex 格式錯誤，可能是 "
                    "DISCORD_PUBLIC_KEY 裡有多餘空白/換行字元）：%s", e)
        return False
    # ↑↑↑ 暫時性除錯 log 結束 ↑↑↑
