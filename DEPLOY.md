# FinFlow 分散式邊緣運算系統部署指南（v16，補齊 Step 0/1/6/7 缺漏，新增 SIT 章節）
...
> Step 4.5 本身沒有就地提醒，容易被漏做，這次補上。**Step 1 目前用 `pip install
> fastapi uvicorn pydantic requests` 而非 `pip install -r requirements.txt`，這點
> 尚未確認是否為刻意設計，暫不更動，待確認後再處理。**
...
**Oracle Linux 上請先做 SELinux relabel**（跟 Step 3 的 `finflow-queue.service` 是
同一類問題，但這是「執行程式」被擋，不是「讀設定檔」被擋，需要另外處理一次，
見文件開頭版本說明的完整解釋）：
